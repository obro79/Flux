...
async def stream_product_data(websocket: WebSocket, product_id: str):
    ...
    try:
        while True:
            ...
            # Raw price data
            for key in [k async for k in r.scan_iter(f"crypto:{product_id}:*")]:
                value = await r.get(key)
                if value is not None:
                    name = key.decode().removeprefix(f"crypto:{product_id}:")
                    data[name] = float(value)

            # Indicators
            for key in [k async for k in r.scan_iter(f"indicator:{product_id}:*")]:
                value = await r.get(key)
                if value is not None:
